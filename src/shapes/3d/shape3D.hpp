#ifndef GEARX_OBJECTS_shape3D_HPP
#define GEARX_OBJECTS_shape3D_HPP
...
/*         class Box 
    ...
    public:
        ...
        :PrismBase(Vec3{0.0f}, height), m_option(option) {
            ...
            assert(radius > 0);
            m_radius = radius;
            Vec3 pos1 = Vec2to3(leftSemiCirclePos);
            Vec3 pos2 = Vec2to3(rightSemiCirclePos);
            // 線の長さは2点間の距離 
            m_line = dist3(pos1, pos2);
            // 物体の位置は２点の中心
            m_pos = (pos1 + pos2)/2.0;
            // 回転角 atan(z, x)
